...
{
	static const std::string joystickDescription = "Unknown JoystickWin32_cls";
...
void JoystickWin32_cls::Initialize()
{
	InitializeInput();
...
void JoystickWin32_cls::Cleanup()
{
	CleanupInput();
}

bool JoystickWin32_cls::IsConnected(unsigned int _JoystickIndex)
{
	if (DirectInput)
	{
...
bool JoystickWin32_cls::Open(unsigned int _JoystickIndex)
{
	if (DirectInput)
	{
...
JoystickData JoystickWin32_cls::GetJoystickData() const
{
	if (DirectInput)
	{
	...
	data.Axes[JoystickAxis_e::Z] = (m_Data.wCaps & JOYCAPS_HASZ) != 0;
	data.Axes[JoystickAxis_e::R] = (m_Data.wCaps & JOYCAPS_HASR) != 0;
	data.Axes[JoystickAxis_e::U] = (m_Data.wCaps & JOYCAPS_HASU) != 0;
	data.Axes[JoystickAxis_e::V] = (m_Data.wCaps & JOYCAPS_HASV) != 0;
	data.Axes[JoystickAxis_e::PovX] = (m_Data.wCaps & JOYCAPS_HASPOV) != 0;
	data.Axes[JoystickAxis_e::PovY] = (m_Data.wCaps & JOYCAPS_HASPOV) != 0;

	return data;
}

JoystickIdentification_e JoystickWin32_cls::GetIdentification() const
{
	return m_Identification
}

JoystickState JoystickWin32_cls::Update()
{
	if (DirectInput)
	{
		if (m_Buffered)
	...
	if (joyGetPosEx(m_JoystickIndex, &pos) == JOYERR_NOERROR)
	{
		...
		state.Axes[JoystickAxis_e::X] = (static_cast<float>(pos.dwXpos) - static_cast<float>(m_JoystickData.wXmax + m_JoystickData.wXmin) / 2.f) * 200.f / static_cast<float>(m_JoystickData.wXmax - m_JoystickData.wXmin);
		state.Axes[JoystickAxis_e::Y] = (static_cast<float>(pos.dwYpos) - static_cast<float>(m_JoystickData.wYmax + m_JoystickData.wYmin) / 2.f) * 200.f / static_cast<float>(m_JoystickData.wYmax - m_JoystickData.wYmin);
		state.Axes[JoystickAxis_e::Z] = (static_cast<float>(pos.dwZpos) - static_cast<float>(m_JoystickData.wZmax + m_JoystickData.wZmin) / 2.f) * 200.f / static_cast<float>(m_JoystickData.wZmax - m_JoystickData.wZmin);
		state.Axes[JoystickAxis_e::R] = (static_cast<float>(pos.dwRpos) - static_cast<float>(m_JoystickData.wRmax + m_JoystickData.wRmin) / 2.f) * 200.f / static_cast<float>(m_JoystickData.wRmax - m_JoystickData.wRmin);
		state.Axes[JoystickAxis_e::U] = (static_cast<float>(pos.dwUpos) - static_cast<float>(m_JoystickData.wUmax + m_JoystickData.wUmin) / 2.f) * 200.f / static_cast<float>(m_JoystickData.wUmax - m_JoystickData.wUmin);
		state.Axes[JoystickAxis_e::V] = (static_cast<float>(pos.dwVpos) - static_cast<float>(m_JoystickData.wVmax + m_JoystickData.wVmin) / 2.f) * 200.f / static_cast<float>(m_JoystickData.wVmax - m_JoystickData.wVmin);

		if (pos.dwPOV != 0xFFFF)
		{
			const float angle = static_cast<float>(pos.dwPOV) / 18000.f * 3.141592654f;
			state.Axes[JoystickAxis_e::PovX] = std::sin(angle) * 100;
			state.Axes[JoystickAxis_e::PovY] = std::cos(angle) * 100;
		}
		else
		{
			state.Axes[JoystickAxis_e::PovX] = 0;
			state.Axes[JoystickAxis_e::PovY] = 0;
		}
...
void JoystickWin32_cls::SetLazyUpdate(bool _Status)
{
	LazyUpdates = _Status;
}

void JoystickWin32_cls::UpdateConnections()
{
	if (DirectInput)
	{
...
void JoystickWin32_cls::InitializeInput()
{
...
void JoystickWin32_cls::CleanupInput()
{
	if (DirectInput)
	{
		DirectInput->Release();
...
bool JoystickWin32_cls::IsConnectedInput(unsigned int _JoystickIndex)
{
...
void JoystickWin32_cls::UpdateConnectionsInput()
{
	...
	const HRESULT result = DirectInput->EnumDevices(DI8DEVCLASS_GAMECTRL, &JoystickWin32_cls::DeviceEnumerationCallback, nullptr, DIEDBSFL_ATTACHEDONLY);
...
bool JoystickWin32_cls::OpenInput(unsigned int _JoystickIndex)
{
	m_Device = nullptr;
...
bool JoystickWin32_cls::CreateDevice(const JoystickRecord& _Record)
{
...
bool JoystickWin32_cls::CheckVendorAndProductIDOfDevice()
{
...
void JoystickWin32_cls::GetFriendlyProductNameOfDevice()
{
...
bool JoystickWin32_cls::SetDeviceDataFormat()
{
...
bool JoystickWin32_cls::CheckDeviceCapabilities()
{
...
bool JoystickWin32_cls::EnumerateDeviceObjects()
{
...
bool JoystickWin32_cls::SetDeviceAxisModeToAbsolute()
{
...
bool JoystickWin32_cls::EnableBufferingBySettingBufferSize()
{
	auto property = DIPROPDWORD();;
	property.diph.dwSize = sizeof(property);
	property.diph.dwHeaderSize = sizeof(property.diph);
...
void JoystickWin32_cls::CloseInputs()
{
	if (m_Device)
	{
		m_Device->Release();
...
JoystickData JoystickWin32_cls::GetJoystickDataInput() const
{
...
JoystickState JoystickWin32_cls::UpdateInputBuffered()
{
	...
	for (DWORD eventIndex = 0; eventIndex < eventCount; eventIndex++)
	{
		...
		while ((!breakLoop) && (axisIndex < JoystickAxisCount))
		{
			...
			{
				if ((axis == JoystickAxis_e::PovX) || (axis == JoystickAxis_e::PovY))
				{
					...
					if (value != 0xFFFF)
					{
						...
						m_State.Axes[JoystickAxis_e::PovX] = std::sin(angle) * 100.f;
						m_State.Axes[JoystickAxis_e::PovY] = std::cos(angle) * 100.f;

					}
					else
					{
						m_State.Axes[JoystickAxis_e::PovX] = 0.f;
						m_State.Axes[JoystickAxis_e::PovY] = 0.f;
					}
				}
				else
				{
...
JoystickState JoystickWin32_cls::UpdateInputPolled()
{
	JoystickState state;
...
BOOL CALLBACK JoystickWin32_cls::DeviceEnumerationCallback(const DIDEVICEINSTANCE* _DeviceInstance, void* _UserData)
{
	bool breakLoop = false;
	unsigned int recordIndex = 0;
...
BOOL CALLBACK JoystickWin32_cls::DeviceObjectEnumerationCallback(const DIDEVICEOBJECTINSTANCE* _DeviceObjectInstance, void* _UserData)
{
	JoystickWin32_cls& joystick = *reinterpret_cast<JoystickWin32_cls*>(_UserData);
	...
	if (DIDFT_GETTYPE(_DeviceObjectInstance->dwType) & DIDFT_AXIS)
	{
		...
		{
		case GUID_XAxis:
			joystick.m_Axes[JoystickAxis_e::X] = DIJOFS_X;
			break;

		case GUID_YAxis:
			joystick.m_Axes[JoystickAxis_e::Y] = DIJOFS_Y;
			break;

		case GUID_ZAxis:
			joystick.m_Axes[JoystickAxis_e::Z] = DIJOFS_Z;
			break;

		case GUID_RzAxis:
			joystick.m_Axes[JoystickAxis_e::R] = DIJOFS_RZ;
			break;

		case GUID_RxAxis:
			joystick.m_Axes[JoystickAxis_e::U] = DIJOFS_RX;
			break;

		case GUID_RyAxis:
			joystick.m_Axes[JoystickAxis_e::V] = DIJOFS_RY;
			break;

		case GUID_Slider:
			if (joystick.m_Axes[JoystickAxis_e::U] == -1)
			{
				joystick.m_Axes[JoystickAxis_e::U] = DIJOFS_SLIDER(0);
			}
			else
			{
				joystick.m_Axes[JoystickAxis_e::U] = DIJOFS_SLIDER(1);
			}
			break;
	...
	if(DIDFT_GETTYPE(_DeviceObjectInstance->dwType) & DIDFT_POV)
	{
		...
		{
			if (joystick.m_Axes[JoystickAxis_e::PovX] == -1)
			{
				joystick.m_Axes[JoystickAxis_e::PovX] = DIJOFS_POV(0);
				joystick.m_Axes[JoystickAxis_e::PovY] = DIJOFS_POV(0);
			}
		}
