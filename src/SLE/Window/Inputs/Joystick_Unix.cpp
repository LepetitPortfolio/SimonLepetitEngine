...
JoystickData Joystick::GetJoystickData() const
{
	...
	for(int axisIndex = 0; axisIndex < axesCount; axisIndex++)
	{
		...
		case ABS_Z:
		case ABS_THROTTLE: 
			data.Axes[JoystickAxis::Z] = true; 
			break;
		
		case ABS_RZ:
		case ABS_RUDDER:   
			data.Axes[JoystickAxis::R] = true; 
			break;
		
		case ABS_RX:       
			data.Axes[JoystickAxis::U] = true; 
			break;
		
		case ABS_RY:       
			data.Axes[JoystickAxis::V] = true; 
			break;
		
		case ABS_HAT0X:    
			data.Axes[JoystickAxis::PovX] = true; 
			break;
		
		case ABS_HAT0Y:    
			data.Axes[JoystickAxis::PovY] = true; 
			break;
	...
	while (result > 0)
	{
		...
		case JS_EVENT_AXIS:
			...
			if (joyState.number < m_Mapping.size())
			{
				...
				{
				case ABS_X:
					m_State.Axes[JoystickAxis::X] = value;
					break;
				
				case ABS_Y:
					m_State.Axes[JoystickAxis::Y] = value;
					break;
				
				case ABS_Z:
				case ABS_THROTTLE:
					m_State.Axes[JoystickAxis::Z] = value;
					break;
				
				case ABS_RZ:
				case ABS_RUDDER:
					m_State.Axes[JoystickAxis::R] = value;
					break;
				
				case ABS_RX:
					m_State.Axes[JoystickAxis::U] = value;
					break;
				
				case ABS_RY:
					m_State.Axes[JoystickAxis::V] = value;
					break;
				
				case ABS_HAT0X:
					m_State.Axes[JoystickAxis::PovX] = value;
					break;
				
				case ABS_HAT0Y:
					m_State.Axes[JoystickAxis::PovY] = value;
					break;
